...
const TaskType INITIALIZE = 1;
const TaskType UI = 2;

extern void TASKNAME( ActionTask )( void );
extern void TASKNAME( INITIALIZE )( void );
extern void TASKNAME( UI )( void );

static __STK_UNIT _stack_ActionTask[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_INITIALIZE[__TCOUNT_STK_UNIT(512)];
static __STK_UNIT _stack_UI[__TCOUNT_STK_UNIT(512)];

const Priority tinib_inipri[TNUM_TASK] = { TPRI_MINTASK + 3, TPRI_MINTASK + 1, TPRI_MINTASK + 1, };
const Priority tinib_exepri[TNUM_TASK] = { TPRI_MINTASK + 3, TPRI_MINTASK + 1, TPRI_MINTASK + 1, };
const UINT8 tinib_maxact[TNUM_TASK] = { (1) - 1, (1) - 1, (1) - 1, };
const AppModeType tinib_autoact[TNUM_TASK] = { 0x00000000, 0x00000001, 0x00000000, };
const FP tinib_task[TNUM_TASK] = { TASKNAME( ActionTask ), TASKNAME( INITIALIZE ), TASKNAME( UI ), };
const __STK_UNIT tinib_stk[TNUM_TASK] = { (__STK_UNIT)_stack_ActionTask, (__STK_UNIT)_stack_INITIALIZE, (__STK_UNIT)_stack_UI, };
...
const AlarmType cyclic_alarm3 = 1;
...
static void _activate_alarm_cyclic_alarm3( void )
{ (void)ActivateTask( UI ); }

const CounterType alminib_cntid[TNUM_ALARM] = { 0, 0, };
const FP alminib_cback[TNUM_ALARM] = { _activate_alarm_cyclic_alarm1, _activate_alarm_cyclic_alarm3, };
const AppModeType alminib_autosta[TNUM_ALARM] = { 0x00000001, 0x00000001, };
const TickType alminib_almval[TNUM_ALARM] = { 1, 1, };
const TickType alminib_cycle[TNUM_ALARM] = { 4, 40, };
...
const EventMaskType RUNEVENT = (1UL << 0);
